...
{{--    指定してあげて、渡してあげる。--}}
    <form action="{{ action('App\Http\Controllers\AccountController@deedUpdateAccount', $datas->id)}}" method="post">
      ...
      <div class="variable_name">
        <input type="text" id="id" name="user_name" class="edit_form" value="{{ old('$datas->user_name', $datas->user_name) }}" placeholder="Submit your Name">
      </div>
        {{--        パスワードは変更する時にのみ入力する。--}}
      ...
      <h3 class="showaccount_email">Account Email</h3>
      <div class="variable_email">
        <input type="email" id="id" name="email" class="edit_form" value="{{ old('$datas->email', $datas->email) }}" placeholder="Submit your Address.">
      </div>
